...
const calibrationPoints=[
  {left:'10%',top:'10%'},{left:'90%',top:'10%'},
  {left:'90%',top:'90%'},{left:'10%',top:'90%'}
];
let calCount=0, calibrated=false, tracking=false, heatmapVisible=false, heatmap;
...
loginBtn.onclick = ()=>{
  if(username.value==='testuser' && password.value==='welcome123'){
    loggedIn=true;
    loginMsg.innerText='Login successful!'; loginMsg.className='text-success';
    document.querySelectorAll('.nav-link.disabled').forEach(el=>el.classList.remove('disabled'));
    setTimeout(()=>new bootstrap.Tab(document.querySelector('[data-bs-target="#ai"]')).show(),300);
    awardBadge('Social Starter');
  } else {
    loginMsg.innerText='Invalid credentials.'; loginMsg.className='text-danger';
  }
};
...
// CALIBRATION
startCal.onclick = ()=>{
  calArea.innerHTML=`<img src="https://via.placeholder.com/600x300" class="w-100 h-100"/>
  ...
  calCount=0; calibrated=false; calMsg.innerText='Click each dot to calibrate.';
  calibrationPoints.forEach(pt=>{
    const d=document.createElement('div');
    d.className='cal-dot'; d.style.left=pt.left; d.style.top=pt.top;
    d.onclick=()=>{
      d.style.background='green'; if(++calCount===4){
        calibrated=true; calMsg.innerText='Calibration complete!'; gazeControls.classList.remove('d-none');
      }
    };
...
function resizeCanvas(){ scanCanvas.width=scanCanvas.clientWidth; scanCanvas.height=200; }
window.addEventListener('resize',resizeCanvas); resizeCanvas();

startGaze.onclick = ()=>{
  if(!calibrated) return alert('Calibrate first');
  tracking=true; stopGaze.disabled=false; startGaze.disabled=true;
  calArea.onmousemove=handleGaze; gazeData=[]; fixations=[];
  scanCtx.clearRect(0,0,scanCanvas.width,scanCanvas.height);
  heatmap = h337.create({container:heatmapContainer, radius:30});
};

stopGaze.onclick = ()=>{
  tracking=false; stopGaze.disabled=true; startGaze.disabled=false;
  calArea.onmousemove=null; updateGazeStats();
};
...
toggleHeatmap.onclick = function(){
  ...
  else heatmapContainer.innerHTML='';
};

let lastPos=null, fixStart=null, lastTime=0;
function handleGaze(e){
  ...
  scanCtx.beginPath();
  scanCtx.arc(x/600*scanCanvas.width,y/300*200,4,0,2*Math.PI);
  scanCtx.fill();
  // fixations
  if(lastPos && tracking){
    if(Math.hypot(x-lastPos.x,y-lastPos.y)<30){
      ...
      fixations.push({x:fixStart.x,y:fixStart.y,dur:lastTime-fixStart.t}); fixStart=null;
    }
  }
  gazeDot.style.display='block'; gazeDot.style.left=`${x}px`; gazeDot.style.top=`${y}px`;
...
function updateGazeStats(){
  ...
  const avgDur=c?Math.round(fixations.reduce((s,f)=>s+f.dur,0)/c):0;
  let sac=0;
...
document.querySelector('[data-bs-target="#recs"]').addEventListener('shown.bs.tab', ()=>{
  ...
  if(riskLevel==='High'){
    recs.push(modePref==='Virtual'?
      'Join a daily online group.':
      'Attend a weekly community meetup.');
  } else if(riskLevel==='Moderate'){
    recs.push(`Schedule ${freqPref.toLowerCase()} chats.`);
  } else {
    recs.push('Keep up the great connections!');
  }
  recs.push(modePref==='Virtual'?
    'Try a senior chat app.':
    'Check your local centre events.');
...
navigator.mediaDevices.getUserMedia({audio:true})
  .then(stream=>{
    ...
    recorder.onstop=()=>{
      ...
      audioMsg.innerHTML=`<p>Recorded audio:</p>
...
setReminder.onclick=()=>{
  ...
  reminderTime.value=''; awardBadge('Planner');
};
